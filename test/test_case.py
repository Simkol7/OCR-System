...
def run_batch_test(test_dir="test_dataset", output_csv="test_results.csv"):
    """
    对测试集中的所有图像执行双方案批量对比测试，输出耗时、字符数和识别文本到CSV报告。
    超大图像（最长边 > 1280px）会等比压缩后再测试，防止内存溢出。
    """
        ...
        for idx, img_name in enumerate(image_files, 1):
            ...
            try:
                text_a, _, time_a = algo_a.detect_and_recognize(
                    target_path, conf_threshold=0.5, erode_size=2, dilate_x=15, dilate_y=3
                )
            ...
            try:
                text_b, _, time_b = algo_b.detect_and_recognize(
                    target_path, conf_threshold=0.5, unclip_ratio=1.5
                )
    ...
    with open(output_csv_path, mode="w", newline="", encoding="utf-8-sig") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=["Image", "Time_A(s)", "Time_B(s)", "CharCount_A", "CharCount_B", "Text_A", "Text_B"],
        )
        writer.writeheader()
        writer.writerows(results)

    avg_time_a = sum(r["Time_A(s)"] for r in results) / len(results)
    avg_time_b = sum(r["Time_B(s)"] for r in results) / len(results)
    print("\n========== 测试完成 ==========")
...
if __name__ == "__main__":
    run_batch_test(
        test_dir=os.path.join(os.path.dirname(os.path.dirname(__file__)), "test_dataset"),
